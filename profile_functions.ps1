...
# function Test-Install($program) {
#   $InstalledSoftwareLM = Get-ChildItem "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall"
#   $InstalledSoftwareLM_names = foreach($obj in $InstalledSoftwareLM){$obj.GetValue('DisplayName')}

#   $InstalledSoftwareCU = Get-ChildItem "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall"
#   $InstalledSoftwareCU_names = foreach($obj in $InstalledSoftwareCU){$obj.GetValue('DisplayName')}

#   $InstalledSoftware = $InstalledSoftwareLM_names + $InstalledSoftwareCU_names

#   $test = foreach ($obj in $InstalledSoftware){$obj.Contains($program)}

#   $out = $test -contains $true
